...
`SchemaMismatch` / `TypeConversion` / `FlussClient` / `Fixture` / `Internal`。
...
| `test-fake` | 无集群集成测试：用 in-memory fake 回放已提交的 fixture，跑通完整 catalog/执行路径（快速绿灯门禁） | 否 |
| `integration_tests` | 真实 Fluss 集群集成测试：capture（录制 fixture）与 e2e（真实 SQL 走真实后端） | 是（Docker / podman） |

常用命令：

```bash
# 单元 + 无集群集成测试（快，CI 默认门禁）
cargo test -p fluss-datafusion --features test-fake

# 真实集群 e2e（需要可用的容器运行时）
cargo test -p fluss-datafusion --features integration_tests -- e2e
```

测试采用三层设计：`capture` 从真实集群录制 fixture，`test-fake` 回放 fixture 做快速校验，
`e2e` 用真实后端跑同一组断言——三者共用同一份建表/灌数逻辑，保证彼此不会对「表长什么样」产生分歧。
