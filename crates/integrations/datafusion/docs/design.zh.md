...
- `FlussSource` trait（`pub(crate)`）只覆盖当前 crate 真正用到的原子操作：metadata 列举/获取、
  KV 完整 primary-key `lookup(key) -> RecordBatch`、分区列举（`list_partitions`）、有界
  `bounded_scan(partition_id, bucket, projection, limit) -> Vec<RecordBatch>`。`bounded_scan`
  同时服务 log 与 KV 的有界 scan——fluss 客户端的 `LimitBatchScanner` 会按表类型分别解码 log/KV
  batch，故同一调用即可支撑两者。它接收 `Option<i64>` 分区 id（非分区表为 `None`），以便 scanner
  定位分区限定的 bucket。
- `real.rs`：唯一实现，包 `FlussConnection` / `FlussAdmin` / lookuper / scanner，复用 fluss
  ...
  分区键/值字符串对）；`bounded_scan` 经 `TableBucket::new_with_partition` 构造 bucket。
...
- `log.rs`：`FlussLogTableProvider` 在 `limit` 为 `None` 时清晰返回 `LimitRequired`；projection
  下推到 `FlussSource::bounded_scan`，`SELECT *` 的完整 identity projection 会归一化为 `None`。
...
- `log_scan.rs`：`FlussLogScanExec`，leaf，每个扫描目标（`(partition_id, bucket)` 对）报告一个
  ...
  异步调 `source.bounded_scan(partition_id, bucket, projection, limit)`。同一 plan 同时支撑 log 与
  KV 的有界 scan，其 `EXPLAIN` 标签由 `plan_name` 字段参数化——log provider 传 `FlussLogScanExec`，
  KV provider 传 `FlussKvScanExec`。
- `stream.rs`：把 lookup / scan 输出适配为 DataFusion stream（`futures::stream::once` /
  Vec 版），drop 即协作式取消。
- 不对称点：`bounded_scan` 在 `FlussSource` 一侧已按 projection 投影，故有界 scan 路径**不再二次投影**；
...
1. **点查**——**完整 primary-key 等值**谓词（`pk1 = v1 AND pk2 = v2 ...`，按 PK 全列）。命中返回
   1 行，未命中返回 0 行（不报错）。
2. **有界 scan**——任何其它谓词形态配合 `LIMIT`。它复用 log 表同一套有界 scan 机制（每 bucket
   末尾 `limit` 行，DataFusion 再施加最终的跨目标 `LIMIT`），`EXPLAIN` 显示为 `FlussKvScanExec`。
   分区表中分区列等值会裁剪被扫描的分区（`Inexact`，残余 `FilterExec` 复核）。

既非完整 PK 等值又不带 `LIMIT` 的查询（如部分 key、非 key filter、或裸 `SELECT *`）一律以
`UnsupportedQueryPattern` 清晰失败，绝不静默退化为无界 full scan。prefix-scan 下推（仅 bucket-key
等值而无完整 PK）暂列为非目标。

分区 KV 表的点查无需额外处理：分区 PK 表的分区列本身就是主键的一部分，完整 PK 等值已绑定它们。
Fluss `Lookuper` 会从 lookup key row 解析出分区 id，因此点查路径会自动定位到唯一所属分区。
