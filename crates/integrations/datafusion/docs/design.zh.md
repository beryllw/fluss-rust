...
2. KV 表支持三种读路径（按优先级）：完整 primary-key 等值谓词下推为点查（point lookup）；bucket-key 前缀等值（分区表还需给出全部分区列等值）下推为 prefix lookup；否则在带 `LIMIT` 时走有界 scan
...
- `FlussSource` trait（`pub(crate)`）只覆盖当前 crate 真正用到的原子操作：metadata 列举/获取、
  ...
  `bounded_scan(partition_id, bucket, projection, limit) -> Vec<RecordBatch>`，以及 log 快照
  `log_scan(partition_id, bucket, projection, row_limit) -> Vec<RecordBatch>`。`bounded_scan`
  ...
  offsets 为止的有限快照。两者都接收 `Option<i64>` 分区 id（非分区表为 `None`），以便 scanner
  定位分区限定的 bucket。
...
- `kv.rs`：`FlussKvTableProvider` 按优先级提供两种读形态。`scan()` 先试 `analyze_kv_filters`：
  完整 PK 等值则构建点查 plan（正确传 projection）；否则若带 `LIMIT`，则复用 `FlussLogScanExec`
  跑有界 KV scan（显示名 `FlussKvScanExec`），其扫描目标与分区裁剪的算法与 log 表完全一致；
  既无完整 PK 等值又无 `LIMIT` 时直接上抛 `UnsupportedQueryPattern`，绝不退化为无界 full scan。
  `supports_filters_pushdown` 是优先级感知的以保证 soundness：仅当**整组** filter 构成点查会真正
  应用的完整 PK 等值时，filter 才标 `Exact`（被消费、不生成 `FilterExec`）；走有界 scan 路径时一律
  不标 `Exact`——分区表对分区列等值标 `Inexact`（驱动裁剪，残余 `FilterExec` 复核），其余 `Unsupported`。
  这避免了「部分 PK 等值 + `LIMIT` 被误标 `Exact` 而被静默丢弃」的不正确情形。
...
既非完整 PK 等值、也非完整 bucket-key 前缀等值、又不带 `LIMIT` 的查询（如部分 key、非 key filter、或裸 `SELECT *`）一律以
`UnsupportedQueryPattern` 清晰失败，绝不静默退化为无界 full scan。
