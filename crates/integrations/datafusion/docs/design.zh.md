...
- `log_scan.rs`：`FlussLogScanExec`，leaf，报告 `num_buckets` 个 partition；`execute(partition)`
  扫描第 `partition` 个 bucket，经 `bounded_batches_stream` 异步调
  `source.log_scan(bucket, projection, limit)`；`EXPLAIN` 显示 `FlussLogScanExec`。
